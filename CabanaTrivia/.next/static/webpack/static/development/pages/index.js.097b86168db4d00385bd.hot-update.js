  ...
  Object(_babel_runtime_corejs2_helpers_esm_createClass__WEBPACK_IMPORTED_MODULE_3__["default"])(QuestionsComponent, [{
    ...
    */
    ...
    value: function onGetQuestions(category) {
      var chosenCategories, allUrls, finalCateg, difficulty, numQs, allDataFinal, temp, json, fetchRequest, i;
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              for (i = 0; i < allUrls.length; i++) {
                fetch(allUrls[i]).then(function (response) {
                  return response.json();
                }).then(temp = this.parseQuestionAnswerFormat(response.results));
                allDataFinal.concat(temp);
              } // fetchRequest = await fetch(allUrls[i])
              //
              // json = await fetchRequest.json();
              // json = this.parseQuestionAnswerFormat(json.results)
              // allDataFinal.concat(json)


              this.setState({
                questionsArr: allDataFinal
              });
...
//# sourceMappingURL=index.js.097b86168db4d00385bd.hot-update.js.map